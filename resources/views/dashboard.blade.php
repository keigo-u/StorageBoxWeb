        ...
        <div class="md:m-3 p-5 w-1/3 h-72 bg-white overflow-hidden shadow-sm md:rounded-lg">
            ...
            <div class="flex justify-between text-slate-400 text-xs my-2">
                <div>
                    <div class="flex">
                        <div>現在の総ページ数：</div>
                        <div class="text-slate-500">{{ $latest_page->count ?? 0 }}</div>
                    </div>
                    <div>取得日時：{{ $latest_page->created_at ?? "未取得" }}</div>
                </div>
                <button class="hover:text-slate-500" onclick="getLatestPageCount()">再取得</button>
            </div>
            <div class="flex justify-end my-2">
                <button class="px-3 py-2 bg-slate-200 hover:bg-slate-300 shadow-md rounded-md" onclick="executeScraping()">実行</button>
            </div>
        </div>
